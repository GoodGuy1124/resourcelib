...
import {BarrettMu,BigInt,biFromHex,biHighIndex,biCopy,biToHex} from './BigInt'
...
// RSACryptoServiceProvider implementation.
{
	...
	/*var b = Str2Bytes(result.substring(0, result.length - 1));
	var s = "";
	for (i = 0; i < b.length; i++) s += b[i] + " ";
	alert(result+"@@"+s);*/

	return Str2Bytes(result.substring(0, result.length - 1));
}
