<template>
  <el-dialog :modal=false top='10%' size="65%" title="选择组" :visible.sync="dialogFormVisible" width="900px">
    <el-row :span='24'  style="width:910px">
      <el-col :span='17'>
        <el-tabs v-model="activeName"  type="card">
          <el-tab-pane label="选择学生用户（组）" name="first"><stumanage></stumanage></el-tab-pane>
          <el-tab-pane label="选择教师用户（组）" name="second"><tcmanage></tcmanage></el-tab-pane>
          <el-tab-pane label="查询" name="third"><groupselect></groupselect></el-tab-pane>
        </el-tabs>
      </el-col>
    <el-col :span='7' class="authority">
      <div style="margin: 25px 0;"></div>
      <div style="margin: 25px 0;"></div>
      <el-col :span='10' :offset='4'><el-button type="success" @click="all">确定</el-button></el-col>
      <el-col :span='10' ><el-button type="warning" @click="notall">取消</el-button></el-col>
      <el-col class="title">已选择的用户(组)</el-col>
      <el-col class="title">
        <select size multiple class="select" v-model='selecteddata'>
          <option class="option" v-for='i in selectdata' v-bind:value='i'>{{i}}</option>
        </select>
      </el-col>
      <el-col >
        <el-col :span='10' :offset='4'><el-button @click='deletedata'>删除</el-button></el-col>
        <el-col :span='10'><el-button @click='emptydata'>清空</el-button></el-col>
      </el-col>     
    </el-col>
  </el-row>
</el-dialog>
</template>
<script>
import Bus from '../../Bus.js';
import stumanage from './stu_manage2.vue';
import tcmanage from './tc_manage2.vue';
import groupselect from './select2.vue';
       export default {
       created() {
       Bus.$on('qwer1', data => {
         this.dialogFormVisible=true;
         this.selectdata=[]; });
       Bus.$on('choosedata2', data => {//对比选中的数据和原有的数据，相同的跳过，不同的追加进去
         for(let i=0,m=1,array=this.selectdata;i<data.length;i++){
           if(!array[0]){this.selectdata=data;break;}
           for(let j=0;j<array.length;j++){
             if(data[i]==array[j]){m=0;break;}
           }
           if(m==1) this.selectdata.push(data[i]);m=1;
         }
       });
       },
       data() {
       return {
        dialogFormVisible: false,
        activeName:'first',
        selectdata:[],
        selecteddata:[],
       }
       },
       methods:{
         all(){
           this.dialogFormVisible = false;
            Bus.$emit('mss1',this.selectdata);
         },
         notall(){
            this.dialogFormVisible = false
         },
           deletedata(){
             for(let i=0,a=this.selecteddata;i<a.length;i++){
               for(let j=0,b=this.selectdata;j<b.length;j++){
                  if(a[i]==b[j]){
                    this.selectdata.splice(j,1);
                    break;
                  }
               }
             }
           },
           emptydata(){
             this.selectdata=[];
           },
           UserGrant_err(state,commit,data){
             alert(data.Status);
           },
           UserGrant_cb(state,commit,data){
              if(data.OK == true){
                             this.$message({
                          type: 'info',
                          message: '授权成功！'
          }); 
              }
           }
       },
       components: {
           stumanage,
           tcmanage,
           groupselect,
       },
       }
       
</script>
<style scoped lang="scss">
  .authority{
      .title{
         margin-top: 10px;
         text-align: center;
         font-size: 20px;
         color:blue;
      }
      .select{
          text-align: center;
          width: 70%;
          height: 200px;
          .option{text-align: left}
      }
      .bottom_btn{
          text-align: center;
      }
  }
</style>